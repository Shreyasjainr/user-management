...
const UserForm: React.FC<UserFormProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newErrors = {
      ...
      countries: formData.countries.length
        ? ""
          ...
          <div className="mt-6">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {formData.countries.map((cid) => (
                <span
                  key={cid}
                  className="inline-flex items-center rounded border px-3 py-1 text-sm text-gray-700 border-gray-500"
                >
        ...
        <div className="flex items-center justify-between border-t border-gray-300 p-4">
          <div>
            <button
              type="button"
              onClick={() =>
                typeof onAddNew === "function" ? onAddNew() : null
              }
    ...
    <form onSubmit={handleSubmit} className="h-[75vh] flex flex-col">
      <div className="flex-1 p-4">
        <div className="space-y-5">
          ...
          >
            <label className="block text-sm mb-1">Select Countries</label>
            <div className="relative">
              <input
                ...
                value={
                  formData.countries.length
                    ? formData.countries
                        .map(
                          (id) => countries.find((c) => c.id === id)?.name || id
                        )
                        .join(", ")
                    : ""
                }
              ...
              />
              {!isView && (
                <span className="absolute py-3 pl-3 right-3 top-1/2 -translate-y-1/2 border-l border-gray-200 text-gray-500 pointer-events-none">
                  <img src={dropSvg} alt="" />
                </span>
              )}
            </div>
